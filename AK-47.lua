		...
		Mouse.Button1Down:connect(function()
			if Can_Shoot and Ammo.Value > 0 and Character.Humanoid.Health > 0 then
				...
				while true do
					...
					Camera.CoordinateFrame = CFrame.new(Camera.Focus.p) * (Camera.CoordinateFrame - Camera.CoordinateFrame.p) * CFrame.Angles(0, 0, 0) * CFrame.new(0, 0, (Camera.Focus.p - Camera.CoordinateFrame.p).magnitude)
...
PointLight3.Color = Color3.new(1, 0.9, 0)
...
table.insert(cors,sandbox(LocalScript11,function()
	...
	function Fire(Mouse)
		local Spread = CFrame.Angles(
			math.rad(math.random(-Settings.Spread, Settings.Spread) / 10),
			math.rad(math.random(-Settings.Spread, Settings.Spread) / 10),
			math.rad(math.random(-Settings.Spread, Settings.Spread) / 10)
		)

		local Origin = Character.Head.Position
		local Direction = (CFrame.new(Origin, Mouse.Hit.p) * Spread).lookVector.unit * Settings.Range
		local Ray = Ray.new(Origin, Direction)
		local Hit, Pos = Workspace:FindPartOnRayWithIgnoreList(Ray, Ray_Ignore)

		-- ⚡ Bullet Trail (Neon Yellow Cylinder)
		...
		bullet.Color = Color3.fromRGB(255, 255, 0) -- Bright Yellow
		bullet.Size = Vector3.new(0.15, 0.15, distance)
		bullet.CFrame = CFrame.new(Origin + direction * (distance / 2), Pos)
		bullet.Name = "BulletTrail"
		bullet.Parent = Ignore_Model

		-- Use a cylinder mesh and rotate it properly
		local mesh = Instance.new("CylinderMesh", bullet)
		bullet.CFrame *= CFrame.Angles(0, math.rad(90), 0) -- Rotate cylinder to align with direction

		game.Debris:AddItem(bullet, 0.1)

		-- 💥 Flash at muzzle (Yellow-Orange Glow)
